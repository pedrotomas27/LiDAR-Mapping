...
def generate_custom_pointcloud(msg):
    custom_points = []
    for point in pc2.read_points(msg, skip_nans=True, field_names=("timestampSec", "timestampNsec", "x", "y", "z", "intensity", "ring")):
        intensity = float(point[5])
        ring = np.uint16(point[6])
        custom_points.append([point[2], point[3], point[4], intensity, ring])
